...
// 🔗 Set axios base URL from Render blueprint (VITE_API_HOST -> https://host)
...
const App = () => {
  ...
  const captureImage = async () => {
    ...
    // Auto rotate, resize, enhance
    imageUrl = await autoProcessImage(imageUrl);

    if (mode === 'source') {
      images.push(imageUrl);
      if (images.length < (barcode ? 1 : 3)) return; // Continue capturing up to 3 photos for single, 1 for barcode
      setApproveImages(true); // Prompt approval
      ...
      setCurrentSnapStackItem(snapStackQueue[snapStackQueue.findIndex(item => item.id === currentSnapStackItem.id) + 1] || null); // Preview next item
    }
  };
