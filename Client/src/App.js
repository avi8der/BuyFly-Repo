...
const App = () => {
  ...
  const { data: deweyItems, refetch: refetchDewey } = useQuery<ProductAnalysis[]>({
    ...
    queryFn: async () => {
      ...
      return searchQuery
        ? await db.dewey.where('identifiedProduct').startsWithIgnoreCase(searchQuery).or('brand').startsWithIgnoreCase(searchQuery).toArray()
        : data;
    },
  });
  ...
  const analyzeMutation = useMutation({
    mutationFn: (data: FormData) =>
      axios.post('/api/source', data, { headers: { 'X-eBay-Api-Key': ebayApiKey } }),
    onSuccess: (data) => {
      ...
      if (data.recommendation === 'GOOD_DEAL') {
        const prompt = window.prompt('Enter purchase price:');
        setPurchasePrice(prompt || '0');
        queueToSnapStack.mutate(data);
      } else {
        setHistory((prev) => [...prev, { ...data, historyType: 'fly', timestamp: new Date() }].slice(-200));
      }
    },
    onError: () => toast({ title: 'Error', description: 'Failed to analyze. Retry or use Google Lens?' }),
  });
  ...
  useEffect(() => {
    const startCamera = async () => {
      if (!isMobile) {
        toast({ title: 'Camera Unavailable', description: 'Mobile-only' });
        return;
      }
      try {
  ...
  // Voice search
  useEffect(() => {
    if (voiceSearch && 'SpeechRecognition' in window) {
      const recognition = new (window.SpeechRecognition || window.webkitSpeechRecognition)();
      recognition.onresult = (event) => {
        setSearchQuery(event.results[0][0].transcript);
        toast({ title: 'Voice Search', description: event.results[0][0].transcript });
  ...
  const captureImage = async () => {
    ...
    if (mode === 'source') {
      ...
      const maxPhotos = barcode ? 1 : (batchMode ? 10 : 3); // Batch mode allows more, single mode 3, barcode 1
  ...
  const autoProcessImage = async (imageUrl) => {
    ...
    // Rotate 90 degrees (adjust based on EXIF if needed)
    ...
    enhanceCtx.fillStyle = 'rgb(255,255,255,0.1)';
    ...
    <div className={`mobile-optimized flex flex-col gap-4 p-4 min-h-screen ${theme === 'dark' ? 'bg-gray-900 text-white' : 'bg-white text-gray-900'}`}>
      <div className="sticky top-0 z-10 bg-opacity-80 backdrop-blur-sm">
        ...
        <div className="flex items-center gap-2 p-2">
          ...
          />
          <Button onClick={() => setVoiceSearch(true)} className="bg-transparent">
            <Mic className="w-6 h-6 text-gray-400" />
          </Button>
          <Button onClick={() => setBatchMode(!batchMode)} className="bg-transparent">
            {batchMode ? 'Single' : 'Batch'}
          </Button>
        </div>
        {mode === 'source' && (
          <Button onClick={() => analyzeMutation.mutate(new FormData())} className="w-full bg-blue-600 text-white rounded mt-2">
            Get Results
          </Button>
        )}
      </div>
              ...
              <Virtuoso
                ...
                itemContent={(index, sale) => (
                  <div className={`border p-2 rounded mb-2 ${theme === 'dark' ? 'bg-gray-800' : 'bg-gray-100'}`}>
                    ...
                    <p><a href={`waze://?ll=${sale.latitude},${sale.longitude}&navigate=yes`} target="_blank" className="text-blue-400 underline flex items-center gap-1"><MapPin className="w-4 h-4" /> {sale.address}</a></p>
      ...
      {mode === 'source' && (
        <>
          <div className="camera-container relative">
            ...
            {isMobile && <button onClick={captureImage} className="capture-button absolute bottom-4 right-4 bg-gradient-to-r from-blue-500 to-purple-600 p-4 rounded-full shadow-lg transition-transform hover:scale-95"><Camera className="w-8 h-8 text-white" /></button>}
          </div>
          <div className="flex gap-2">
            <input type="text" value={barcode || ''} onChange={(e) => setBarcode(e.target.value)} placeholder="Barcode" className={`touch-friendly border rounded p-2 flex-1 ${theme === 'dark' ? 'bg-gray-800 text-white' : 'bg-gray-100 text-gray-900'}`} />
            ...
            <input type="text" value={color} onChange={(e) => setColor(e.target.value)} placeholder="Color" className={`touch-friendly border rounded p-2 w-24 ${theme === 'dark' ? 'bg-gray-800 text-white' : 'bg-gray-100 text-gray-900'}`} />
            <input type="text" value={size} onChange={(e) => setSize(e.target.value)} placeholder="Size" className={`touch-friendly border rounded p-2 w-24 ${theme === 'dark' ? 'bg-gray-800 text-white' : 'bg-gray-100 text-gray-900'}`} />
            <input type="text" value={sku} onChange={(e) => setSku(e.target.value)} placeholder="SKU" className={`touch-friendly border rounded p-2 w-24 ${theme === 'dark' ? 'bg-gray-800 text-white' : 'bg-gray-100 text-gray-900'}`} />
            <input type="number" value={quantity} onChange={(e) => setQuantity(Number(e.target.value))} placeholder="Qty" className={`touch-friendly border rounded p-2 w-24 ${theme === 'dark' ? 'bg-gray-800 text-white' : 'bg-gray-100 text-gray-900'}`} />
          </div>
          {analyzeMutation.data && (
            <div className={`border p-4 rounded-lg ${theme === 'dark' ? 'bg-gray-800' : 'bg-gray-100'}`}>
              ...
              <div className="flex gap-2 mt-2">
                {analyzeMutation.data.recommendation === 'GOOD_DEAL' && <Button onClick={() => handleBuy(analyzeMutation.data)} className="bg-green-600 text-white rounded flex items-center gap-2 transition-transform hover:scale-95"><ShoppingCart className="w-5 h-5" /> Buy</Button>}
                <Button onClick={() => setImage(null)} className="bg-gray-600 text-white rounded flex items-center gap-2 transition-transform hover:scale-95">Fly</Button>
              </div>
            </div>
          )}
        </>
        ...
        <>
          {snapStackQueue.length > 0 && !currentSnapStackItem ? (
            <div onTouchStart={handleRefresh}>
              ...
              <div className="flex gap-2 mt-2">
                <Button onClick={() => setCurrentSnapStackItem(snapStackQueue[0])} className="bg-blue-600 text-white rounded transition-transform hover:scale-95">Proceed</Button>
                <Button onClick={async () => { await db.snapStack.clear(); setSnapStackQueue([]); setMode('source'); }} className="bg-red-600 text-white rounded transition-transform hover:scale-95">Start New</Button>
                <Button onClick={sendAllToDewey} className="bg-purple-600 text-white rounded flex items-center gap-2 transition-transform hover:scale-95"><CheckCircle className="w-5 h-5" /> Send All</Button>
              </div>
              <Virtuoso style={{ height: '50vh' }} data={snapStackQueue} itemContent={(index, item) => (
            ...
            <div>
              ...
              {isMobile ? (
                <div className="camera-container relative">
                  <video ref={videoRef} autoPlay playsInline className="camera-video w-full h-[60vh] rounded-lg" />
                  <button onClick={captureImage} className="capture-button absolute bottom-4 right-4 bg-gradient-to-r from-blue-500 to-purple-600 p-4 rounded-full shadow-lg transition-transform hover:scale-95"><Camera className="w-8 h-8 text-white" /></button>
                </div>
              ) : <p className="text-center">Camera is mobile-only</p>}
              <div className="mt-4">
                <p>Photos: {currentSnapStackItem.photos?.length || 0}/25</p>
                <Button onClick={handleNextItem} className="bg-blue-600 text-white rounded flex items-center gap-2 mt-2 transition-transform hover:scale-95">Next</Button>
              </div>
            </div>
      ...
      {mode === 'dewey' && (
        <div onTouchStart={handleRefresh}>
          ...
          {deweyItems ? (
            <Virtuoso style={{ height: '50vh' }} data={deweyItems} itemContent={(index, item) => (
              <div className={`border p-2 rounded mb-2 ${theme === 'dark' ? 'bg-gray-800' : 'bg-gray-100'}`}>
                <p>{item.identifiedProduct} - ${item.estimatedProfit.toFixed(2)}</p>
          ...
          {deweyItems?.length && <Button onClick={triggerVendooBot} className="bg-blue-600 text-white rounded flex items-center gap-2 mt-4 transition-transform hover:scale-95"><CheckCircle className="w-5 h-5" /> Send to Bot</Button>}
        </div>
      )}
      ...
      {mode === 'shipping' && (
        <div onTouchStart={handleRefresh}>
          ...
          {shippingItems ? (
            <Virtuoso style={{ height: '50vh' }} data={shippingItems} itemContent={(index, item) => (
              <div className={`border p-2 rounded mb-2 ${theme === 'dark' ? 'bg-gray-800' : 'bg-gray-100'}`}>
                ...
                <Button onClick={() => markShipped.mutate({ id: item.id, platform: item.platform })} className="bg-green-600 text-white rounded mt-2 transition-transform hover:scale-95">Mark Shipped</Button>
              </div>
            )} />
